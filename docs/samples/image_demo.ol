## image.loadData — charge une image PNG EMBARQUÉE (base64), sans upload.
...
var img = image.loadData("png", SMILEY)   ## décode le base64 → texture

## quelques smileys rebondissants
global sprites = []
for i = 1, 10 do
    sprites[i] = {
        x:  math.randInt(0, W - 64),
        y:  math.randInt(0, H - 64),
        vx: math.rand(-140, 140),
        vy: math.rand(-140, 140),
        s:  math.randInt(28, 72)
    }
end
