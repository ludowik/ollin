...
- **Caméra** : classe native `Camera` (patron `makeClass` comme `Color`) ; `graphics.camera(...)` renvoie une INSTANCE (map à `__class__` portant `px,py,pz, tx,ty,tz, fovy`). Méthodes d'instance (mutent self, renvoient self) : `set_pos`, `look_at`, `move` (translate position+cible), `orbit(angle rad, rayon [, hauteur])`. Comme une instance reste un T_MAP, `cameraFromMap()` la relit sans changement (up défaut +Y, projection perspective).
