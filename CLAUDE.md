...
- `FOR_LOOP ctl, →corps` : `i += pas` (avec garde anti-débordement int64 : si `i+pas` déborde → fin) ; si dans la limite (`≤` si pas>0, `≥` sinon) → `R[ctl]=i`, saut vers le corps ; sinon sortie. `bind(var1, ctl)` copie `i` dans la variable en tête de corps (modifier la variable dans le corps n'affecte pas l'itération).  
