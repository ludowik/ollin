...
- **Transparence (eau)** : à l'enregistrement, `endChunk` **scinde** les cubes en 2 groupes selon l'alpha de la couleur d'instance (`col.a < 250` → groupe transparent) et renvoie `{id, idw, count, wcount}`. `drawChunk` dessine l'opaque (`id`) ; `graphics.drawChunkAlpha(handle)` dessine le groupe transparent (`idw`) en `BLEND_ALPHA` (depth test+write gardés → surface d'eau qui s'occlude proprement, pas d'accumulation entre couches). **Ordre obligatoire** côté script : dessiner TOUT l'opaque (boucle `drawChunk`) PUIS TOUTE l'eau (boucle `drawChunkAlpha`) dans le même `begin3d`. `freeChunk` libère les deux groupes.
