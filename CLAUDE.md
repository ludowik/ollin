...
- **Transformations 3D** : `translate(x,y[,z])`, `rotate(deg[,ax,ay,az])`, `rotateX/Y/Z(deg)`, `scale(s|sx,sy|sx,sy,sz)` pilotent la pile rlgl. Elles affectent les solides instanciés **uniquement encadrées par `push`/`pop`** (rlgl n'écrit dans `RLGL.State.transform` — ce que lit `rlGetMatrixTransform()` — qu'entre `rlPushMatrix`/`rlPopMatrix` ; hors push/pop elles écrivent dans la modelview). Le MVP du flush utilise `s_view3d` (vue figée au `begin3d`), **pas** la modelview courante → une transfo « nue » ne décale pas rétroactivement tous les buckets.
