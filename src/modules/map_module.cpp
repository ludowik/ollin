...
// `for k, v in m`) — a global "map" module, alongside `data`, `string` and the rest.
//
// A map value does NOT get these as pseudo-methods (`m.keys()`), unlike an array or a string:
// every OTHER module (math, string, graphics…) IS a map too, and CALL_METHOD does not inject self
// for a map receiver precisely so that `math.sin(x)` does not hand `math` to `sin` as an extra
// argument (the one narrow exception, the built-in `len`, is matched by its own function pointer
// in vm.cpp — see the comment there). A free function taking the map explicitly, `map.keys(m)`,
// has no such ambiguity and needs no exception.
